"""
Unit tests of the module neuro.core.files.text
"""

import deepdiff
import filecmp
import json
import pytest


class TestText:
    def test_read(self, test_file):
        from neuro.core.file import text
        from neuro.utils.test_utils import get_hash
        data_path = test_file.get("input/files/text.txt")
        with text.Text(data_path) as t:
            txt = t.get_text()

        assert get_hash(txt) == "d15791ad718b73be8fb9013a4292dcc1aa469f04"

    def test_path(self):
        from neuro.core.file import text
        with pytest.raises(FileNotFoundError):
            text.Text(";")


class TestTextCsv:
    def test_is_identifier(self, test_file):
        from neuro.core.file import text
        with text.TextCsv(test_file.get("input/files/text_csv_short.csv")) as t:
            identifier = t.is_identifier("policyID")
            assert identifier is True
            identifier = t.is_identifier("statecode")
            assert identifier is False

    def test_to_json(self, test_file):
        from neuro.core.file import text
        json_path = test_file.create("output/files/text_json_from_csv.json")
        result_json_path = test_file.get("results/files/text_json_from_csv.json")
        with text.TextCsv(test_file.get("input/files/text_csv_short.csv")) as t:
            t.to_json("policyID", json_path, mode="local")
        assert filecmp.cmp(json_path, result_json_path)

    def test_merge_files(self, test_file):
        from neuro.core.file import text
        file_paths = test_file.multi("input/files/text_json_merge")
        merged_path = test_file.create("output/files/text_json_merged.json")
        result_path = test_file.get("results/files/text_json_merged.json")
        text.TextJson.merge_files(file_paths, merged_path)
        assert deepdiff.DeepDiff(json.load(open(merged_path)), json.load(open(result_path)), ignore_order=True) == {}

    def test_insert_layer(self, test_file):
        from neuro.core.file import text
        file_path = test_file.get("input/files/text_json_insert.json")
        inserted_path = test_file.path("output/files/text_json_insert_output.json")
        result_path = test_file.get("results/files/text_json_insert_output.json")
        with text.TextJson(file_path) as t:
            t.insert_layer("mirwalk", inserted_path)

        assert filecmp.cmp(inserted_path, result_path)
